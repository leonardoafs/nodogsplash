...
Após a instalação completa, você deve transferir o arquivo `login_final.sh` para a pasta `/usr/lib/nodogsplash`, renomeando-o para `login.sh`. Esse arquivo promove a mudança de paradigma de Login. Da primeira vez na conexão, abre um Captive Portal pedindo seus dados. Ao colocá-los, você é inscrito no sistema e tem sua primeira presença confirmada. A partir da segunda vez, o Captive Portal abre, mostra seus dados e pede para que o aluno clique em "Continuar", para efetivamente ganhar acesso à internet.
