...
#For Raspbian:
#mountpoint="/home/pi/teste"
#logdir="/home/pi/teste/ndslog/"
#logname="ndslog.log"
...
if grep -q "$mac" /home/pi/lista_macs.txt
then
	INPUT="/home/pi/presenca.csv"
	OLDIFS=$IFS
	IFS=','
	[ ! -f $INPUT ] && { echo ""; exit 99; }
	...
	<div class=\"left-part\">
		...
		<h1>Sistema de Presenca Virtual UFRJ</h1>
		<h2>OBRIGADO!</h2>
		<h3>Nome: $nome_final</h3>
		<h3>Email: $email_final</h3>
		<h3>DRE: $dre_final</h3>
		<h4>Sua presença foi computada com SUCESSO! Clique em \"Continuar\" para sair dessa tela.</h4>"
	echo "<hr>"
	...
	echo "</form><hr>"
	echo "</div></div><br>"
	#echo "<p><italic-black>Obrigado!</italic-black></p>"
	#echo "<p><italic-black>SUA PRESENÇA FOI COMPUTADA COM SUCESSO! Clique em Continuar para sair dessa tela!</italic-black></p>"
	#echo "</div>"
	#echo "<form action=\"/nodogsplash_auth/\" method=\"get\">"
	#echo "<input type=\"hidden\" name=\"tok\" value=\"$tok\">"
	#echo "<input type=\"hidden\" name=\"redir\" value=\"$requested\"><br>"
	...
	echo "$mac,$nome_final,$email_final,$dre_final,$(date);" >> /home/pi/presenca.csv
	...
	if [ -z "$username" ] || [ -z "$emailaddr" ]; then
		...
		echo "<br>Seja bem vindo, <b>$usernamehtml</b>!<br>"
		...
		echo "<br><italic-black> Seu cadastro foi <b>confirmado com sucesso</b>.<br> Obrigado!</italic-black>"
		...
		echo "$mac,$usernamehtml,$emailaddr,$dre,$(date);" >> /home/pi/presenca.csv

		echo "$mac" >> /home/pi/lista_macs.txt
